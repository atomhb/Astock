...
def configure_matplotlib_chinese_font() -> None:
    """Detect a CJK font for Matplotlib; GitHub Actions should install fonts-noto-cjk."""
    candidates = [
...
CONFIG = {
    ...
    # ── 下跌趋势风险控制（只使用原策略已有的沪指 MA5 / MA20 信息）──
    "bear_market_pos_ratio": 0.30,         # MA5 < MA20*0.98：原策略的熊市仓位上限
    "deep_bear_market_pos_ratio": 0.10,    # MA5 < MA20*0.95：深度下跌，仅保留10%防御仓位
    "crash_market_pos_ratio": 0.00,        # MA5 < MA20*0.90：极端下跌，清至现金
    "deep_bear_ma20_ratio": 0.95,
    "crash_ma20_ratio": 0.90,
    "market_rebalance_tolerance_yuan": 1000.0,  # 超出目标仓位不足该金额时不交易，避免小额频繁换手
}
...
def get_market_target_position_ratio(con, trade_date: date, index_symbol="000001.SH") -> float:
    """
    只以原策略已有的沪指 MA5 / MA20 判断大盘状态并确定组合总仓位上限。

    强趋势：MA5 >= MA20                    -> 100%
    震荡/弱趋势：MA5 >= MA20 * 0.98         -> 50%
    普通熊市：MA5 < MA20 * 0.98             -> 30%
    深度熊市：MA5 < MA20 * 0.95             -> 10%
    极端下跌：MA5 < MA20 * 0.90             -> 0%

    这不引入新的选股因子或外部信息；仅把原有的大盘多级仓位控制
    从“限制新买入”升级为可在下跌趋势中触发强制降仓的风险上限。
    """
    ...
    if ma_ratio >= 0.98:
        return 0.50
    if ma_ratio >= float(CONFIG["deep_bear_ma20_ratio"]):
        return float(CONFIG["bear_market_pos_ratio"])
    if ma_ratio >= float(CONFIG["crash_ma20_ratio"]):
...
def enforce_market_risk_rebalance(con, trade_date: date) -> List[Tuple]:
    ...
    原策略仅在 process_pending_orders() 中限制“新买入”，导致市场由强转弱后，
    已有持仓仍可能远高于30%而继续暴露。该函数在个股卖出规则之后、买入之前运行：
    按“收益更弱 + 跌离MA20更深”的顺序卖出，直到总股票市值回到目标上限。
    """
    target_ratio = get_market_target_position_ratio(con, trade_date)
    _, total_assets, available_cash = get_account_state(con)
    if total_assets <= 0:
        return []

    holdings = con.execute("""
        SELECT symbol, buy_price, shares
        FROM virtual_portfolio
    """).df()
    if holdings.empty:
        return []

    raw_today = con.execute(f"""
        SELECT symbol, close
        FROM {STOCKS_TABLE}
        WHERE tradedate = ?
    """, [trade_date]).df()
    ...
    raw_price_map = dict(zip(raw_today["symbol"], raw_today["close"]))

    holdings["close"] = holdings["symbol"].map(raw_price_map)
    holdings = holdings.dropna(subset=["close"]).copy()
    if holdings.empty:
        return []
    holdings["close"] = holdings["close"].astype(float)
    holdings["shares"] = holdings["shares"].astype(int)
    holdings["market_value"] = holdings["close"] * holdings["shares"]
    current_market_value = float(holdings["market_value"].sum())
    target_market_value = float(total_assets) * target_ratio
    excess_value = current_market_value - target_market_value
    tolerance = float(CONFIG.get("market_rebalance_tolerance_yuan", 1000.0))
    ...
    history_start = (trade_date - timedelta(days=45)).strftime("%Y-%m-%d")
    trend_df = con.execute(f"""
        ...
        FROM x
        WHERE date = ?
    """, symbols + [history_start, trade_date.strftime("%Y-%m-%d"), trade_date]).df()

    trend_map = {
        row["symbol"]: (float(row["close"]), float(row["ma20"]) if pd.notna(row["ma20"]) else float(row["close"]))
        for _, row in trend_df.iterrows()
    }
    holdings["pnl_pct"] = (holdings["close"] - holdings["buy_price"].astype(float)) / holdings["buy_price"].astype(float) * 100.0
    holdings["ma20_dev"] = holdings.apply(
        lambda r: (trend_map.get(r["symbol"], (r["close"], r["close"]))[0]
                   / max(trend_map.get(r["symbol"], (r["close"], r["close"]))[1], 1e-8) - 1.0),
        axis=1,
    )
    ...
    log.warning(
        f"🚨 大盘风险降仓触发: 目标仓位={target_ratio*100:.0f}%，"
        f"当前股票市值=¥{current_market_value:,.0f}，"
        f"需降低约¥{excess_value:,.0f}。"
    )

    for _, row in holdings.iterrows():
        if reduced_value >= excess_value:
            break
        symbol = row["symbol"]
        price = float(row["close"])
        shares = int(row["shares"])
        stock_value = price * shares
        remaining = excess_value - reduced_value

        if stock_value <= remaining or remaining / max(stock_value, 1e-8) >= 0.60:
            sell_shares = shares
        else:
            sell_shares = int(remaining / (price * 100.0)) * 100
            sell_shares = max(100, min(sell_shares, shares))

        gross_cash = round(sell_shares * price, 2)
        sell_fee = round(gross_cash * sell_fee_rate, 2)
        net_cash = round(gross_cash - sell_fee, 2)
        pnl_pct = round(float(row["pnl_pct"]), 2)

        if sell_shares >= shares:
            con.execute("DELETE FROM virtual_portfolio WHERE symbol=?", [symbol])
        else:
            con.execute("UPDATE virtual_portfolio SET shares = shares - ? WHERE symbol=?", [sell_shares, symbol])

        con.execute("""
            INSERT INTO trade_history(symbol, trade_type, signal_date, trade_date, price, shares, reason, pnl_pct, fee)
            VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?)
        """, [symbol, TRADE_SELL, trade_date, price, sell_shares, REASON_MARKET_RISK_REBALANCE, pnl_pct, sell_fee])

        available_cash += net_cash
        reduced_value += gross_cash
        sold_rows.append((symbol, sell_shares, price, pnl_pct))
        log.warning(f"🛡️ 强制降仓卖出 {symbol}: {sell_shares}股 @¥{price:.2f}，盈亏{pnl_pct:+.2f}%")
